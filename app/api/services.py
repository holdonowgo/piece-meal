...
def get_users():
    ...
    print("users_schema = models.UserSchema(many=True)")
    users = models.User.query.all()
    print("users = models.User.query.all()")
    return users_schema.dump(users).data
    # print("return users_schema.dump(users).data")
...
def get_alt_ingredients(_id):
    ingredients_schema = models.IngredientSchema(many=True, exclude=['alternatives', 'ingredient_alternative'])
    ingredient = models.Ingredient.query.filter(models.Ingredient.id == _id).one()
    return ingredients_schema.dump(ingredient.alternatives.all()).data


def add_alt_ingredient(ingredient_id, alt_ingredient_id):
    ingredient = models.Ingredient.query.filter(models.Ingredient.id == ingredient_id).one()
    alt_ingredient = models.Ingredient.query.filter(models.Ingredient.id == alt_ingredient_id).one()
    ingredient.add_alternative(alt_ingredient)
    db.session.commit()
